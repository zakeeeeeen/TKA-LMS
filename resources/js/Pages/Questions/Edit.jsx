...
export default function Edit() {
    const { question, subjects, questionTypeOptions = [] } = usePage().props;
    const { data, setData, post, errors, processing } = useForm({
        _method: 'put',
        subject_id: question.subject_id,
        grade_level: question.grade_level ?? 'SD',
        ...
        matrix_left_label: question.matrix_left_label ?? 'Benar',
        matrix_right_label: question.matrix_right_label ?? 'Salah',
        matrix_rows: question.matrix_rows?.length ? question.matrix_rows : [
            { statement: '', correct_answer: 'left' },
            { statement: '', correct_answer: 'left' },
        ],
        explanation: question.explanation ?? '',
    });
    const [imagePreviewUrl, setImagePreviewUrl] = useState(question.image_url ?? null);
    ...
    const hasOptionContent = (option, textValue = data[`option_${option}`]) => {
        return textValue.trim() !== ''
            || Boolean(data[`option_${option}_image`])
            || Boolean(question[`option_${option}_image_url`]);
    };
    ...
    useEffect(() => {
        if (!data.image) {
            setImagePreviewUrl(question.image_url ?? null);
            return undefined;
        }
            ...
            <div className="max-w-4xl mx-auto">
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 space-y-4">
                    ...
                    <QuestionImageUpload
                        ...
                        error={errors.image}
                        fileName={data.image?.name ?? (question.image_url ? 'Gambar tersimpan' : null)}
                    ...
                    {isMatrix ? (
                        ...
                        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                            Tipe isian lama hanya dipertahankan untuk kompatibilitas data lama.
                        </div>
                    ) : (
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div key={option}>
                                        ...
                                        <div className="mt-3">
                                            <QuestionImageUpload
                                                ...
                                                previewUrl={optionImagePreviewUrls[option] ?? question[`option_${option}_image_url`] ?? null}
                                                error={errors[`option_${option}_image`]}
                                                fileName={data[`option_${option}_image`]?.name ?? (question[`option_${option}_image_url`] ? 'Gambar tersimpan' : null)}
