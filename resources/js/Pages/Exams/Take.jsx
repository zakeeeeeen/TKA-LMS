...
export default function Take() {
    const { exam, questions, answers: initialAnswers } = usePage().props;
    ...
    const [remainingSeconds, setRemainingSeconds] = useState(() => {
        ...
        const target = new Date(exam.end_time).getTime();
        ...
        const timer = window.setInterval(() => {
            setRemainingSeconds((value) => {
                if (value <= 1) {
                    ...
                    router.post(route('exams.finish', exam.id));
                    return 0;
                }
    ...
    const saveAnswer = async (questionId, payload, successText = 'Jawaban tersimpan.') => {
        ...
        try {
            const response = await fetch(route('exams.answer', exam.id), {
                method: 'POST',
                headers: {
            ...
            >
                <div className="p-6">
                    ...
                    <p className="mt-2 text-sm text-slate-600">
                        Apakah Anda yakin ingin menyelesaikan ujian ini? Pastikan Anda telah memeriksa seluruh jawaban Anda.
                    </p>
