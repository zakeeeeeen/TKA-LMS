...
export default function Review() {
    ...
    const sendChat = async (questionId, message, appendUserMessage = true) => {
        ...
        try {
            const response = await fetch(route('results.questions.chat', [result.id, questionId]), {
                method: 'POST',
                headers: {
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="text-2xl font-bold text-slate-900">
                        {exam?.title ?? 'Pembahasan Ujian'}
                    </div>
                    <div className="mt-2 text-sm text-slate-500">
                        Siswa: {result.user?.name ?? '-'} ({result.user?.email ?? '-'})
                    </div>
                </div>
                                        ...
                                        <div>
                                            {(question.explanation ?? '').trim() && (
                                                <div className="border-b border-slate-100 px-6 py-5 bg-slate-50/50">
                                                    <div className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Penjelasan Soal</div>
                                                    <MathContent
